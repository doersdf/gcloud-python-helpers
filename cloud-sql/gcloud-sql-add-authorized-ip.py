...
# Sample: 'gcloud-sql-add-authorized-ip add 10.10.10.10/24 sql-server-name'
...
if len(sys.argv)==4:
    newIP = sys.argv[2]
    command = sys.argv[1]
    serversql = sys.argv[3]
    if pattern.match(newIP)==None:
        print("[203] Error: Invalid IP.\n   Use IP CIDR notation Sample 'gcloud-sql-add-authorized-ip add 192.168.0.1/24 sql-server-name'' ")
    elif command!="add" and command!="remove":
        print("[202] Error: Invalid command.\n   Use Add or Remove 'gcloud-sql-add-authorized-ip add 192.168.0.1/24 sql-server-name'' ")
    else: 
        #call google cloud sql api to get configuration in json format
        
        p = subprocess.Popen("gcloud sql instances describe " + serversql + " --format json", shell=True, stdout=subprocess.PIPE)
        p.wait()
        ...
        if (command =="add" and IPfound==False) or (command=="remove" and IPfound==True):
            if command=="add":
                ...
                ips = ips[1:]
            p = subprocess.Popen("gcloud sql instances patch " + serversql + " --authorized-networks=" + ips, shell=True, stdout=subprocess.PIPE)
            p.wait()
    ...
    print("[201] Error: 3 argument required.\n   Usage 'gcloud-sql-add-authorized-ip.py add 192.168.0.1/24 server-name")
